 ...
 * paste and half a second for the 3000-line one ticket 11's warning exists for.
 * The snippet may be hundreds of lines, and rendering on every keystroke would
 * do all of that once per character and throw all but the last result away.
 * This is the same debounce the detection rides on, which is the point: one
 * source change, one pass.
 *
...
/**
 * Wires up a popup document and starts the work that opening it implies: the
 * language dropdown, the settings and theme reads, the load-time render, the
 * right-click prefill, and the four listeners below.
 *
 * All of it used to be this module's side effects, which made an import the
 * only way in - so it could not be run twice, and it took the clock from
 * whatever global happened to be around it. A test of this file needs both, and
 * neither was a property worth keeping: the document and the debounce's two
 * timer functions are arguments now, each defaulting to the page's own, and the
 * order of everything below is otherwise untouched.
 *
 * Deliberately one function rather than a handful of exported handlers. The
 * class of bug this file has actually had is a wiring bug - three `input`
 * listeners registered by work that could not see itself, and a prefill that
 * replayed each of them by hand - and handlers exported one at a time would let
 * a test assert every one of them while the wiring between them, which is the
 * part that broke, stayed unasserted.
 *
 * @param {object} [host]
 * @param {Document} [host.document] The document to wire up.
 * @param {typeof globalThis.setTimeout} [host.setTimeout] The debounce's clock,
 *   taken as an argument so that a test can hold it still rather than wait out
 *   a real 150ms per render.
 * @param {typeof globalThis.clearTimeout} [host.clearTimeout]
 */
export function startPopup({
  ...
  // The window the popup is closed through, read off the document rather than
  // taken from the global for the same reason as everything else here.
   ...
   * stylesheet has to have finished parsing - but it does not depend on
   * anything the user does, so kicking it off now means the wait has almost
   * always already elapsed by the time Insert is pressed.
   *
  ...
  function fillLanguageDropdown() {
    ...
    for (const { id, label } of options) {
      // Built through the supplied document rather than the `Option`
      // constructor, which would always take the page's own.
      const option = document.createElement("option");
      option.value = id;
      option.textContent = label;
      languageField.add(option);
    }
  }
  ...
  async function insert() {
    ...
    const { html, text } = buildCodeBlockHtml({
      ...
      // that would have filled it in, and this asks for detection in that
      // window rather than shipping a block under a language nobody chose.
      // Asking without honouring leaves the request standing, so the render
      // that was already due still detects.
    ...
    if (!isPlainText) {
      // Set before the block goes in, never after. The default `"auto"` sends
      // an HTML message as plain text when it sees no formatting, which would
      // drop the block entirely; `"both"` also guarantees the plain-text
      // alternative part. Doing it first means a failure here costs an insert
      // rather than leaving an already-inserted block on a message that will
      // downgrade it.
      //
      // Which is only safe because a body-less call leaves the document alone,
      // and that is worth citing rather than assuming: the spec's blanket
      // "every call replaces the whole document, moves the caret to the top and
      ...
      // refreshes the send-format menu. So passing `deliveryFormat` alone
      // cannot touch the caret this insert is about to read - the only marks it
      // leaves are `gContentChanged = true`, on a message we are about to
      // change anyway, and a `focus()` back onto whatever was focused.
    ...
    // downgrade. Ticket 02 predicted this call would be rejected on a
    // plain-text composer, which the popup would then surface as an error while
    // inserting nothing - the button looking broken in exactly the window this
    // ticket is about. Not making the call is both the fix and the honest
    // description.
   ...
   * the insert path read the warning - emailing three thousand lines of code is
   * a mistake worth mentioning and not one worth preventing. Ticket 02 removed
   * the last thing that gated Insert on the textarea's contents; this is not
   * quietly putting one back, and there is no size at which it starts to.
   *
   ...
   * as `y`, because there is one `detectedLanguage` and one `html` and they
   * came out of the same call over the same source.
   *
   * The popup asks for detection the way any caller does - by naming no
   * language - and reads back `detectedLanguage`, which is the language that
   * was applied and not the one that was requested. Detection itself lives
   * behind the seam, and this file neither knows nor can tell that
   * `hljs.highlightAuto` is involved.
   *
   * The rest is the preview: `html` here is not a rendering *like* the one that
   * gets inserted, it is the string that will be. The seam is pure, so the same
   * source, language, theme map and settings cannot produce two different
   * blocks - which is why the preview can be trusted, and why there is
   * deliberately no preview stylesheet and no simplified preview markup
   * anywhere in this popup. A second rendering path would be a second thing to
   * keep correct, and its drift would show up as a preview that was accurate
   ...
   *   escapes every `&`, `<` and `>` in the source before it becomes markup - a
   *   test pins that - so pasted markup arrives as text. This is the guarantee
   *   that matters, and it is the same one the message body already relies on.
   * - It is parsed inertly, by `DOMParser` into a detached document, and only
   *   the resulting block element is adopted. A parse is not an execution: no
   *   script runs, no `src` is fetched, no handler attribute is honoured, and
   *   that holds whatever the string turns out to contain. `innerHTML` on the
   *   live document would be one line shorter and would also fetch an
   *   `<img src>` if the seam ever emitted one.
   * - The popup is an extension page under the default MV3 CSP, so inline
   *   script could not run here even if something managed to write it in.
   */
  async function renderFromSource() {
    ...
    // A newer render was scheduled while this one waited. Dropping the stale
    // one keeps an older render from being the one left on screen: with a
    // debounce in front this is close to unreachable, but "close to" is not a
    // property worth relying on for the element whose whole job is to be
    // accurate.
    ...
    // Assigning `value` is safe for any result: detection can only return a
    // name `hljs.listLanguages()` carries, and that is the same list the
    // dropdown was filled from. It fires no `change`, so writing it here cannot
    // be mistaken for the user taking the language over.
   ...
   * preview - registered by three tickets that could not see each other, and
   * the prefill below had to replay each of them by hand. One entry point means
   * the prefill announces a change instead of re-enacting one, and means the
   ...
   *   debounce. Typing is the debounced case and everything else is not:
   *   content that arrives all at once has no burst to collapse.
   */
  function handleSourceChanged({ wholesale, immediate = false }) {
    // Not debounced, and cheap enough not to be: counting lines is a scan, not
    // a highlight, and a warning that appeared a fifth of a second after the
    // paste would read as a reaction to whatever the user did next.
    refreshSizeWarning();
   ...
   * also the honest reading of the story - the language is detected when code
   * is pasted, and a snippet being tweaked afterwards has already got one - and
   * it is what keeps the dropdown from re-guessing under someone's fingers
   * while they fix a typo.
   *
   * An event with no `inputType` at all counts as wholesale. A browser that
   * will not say what happened should cost a redundant detection, not a
   * dropdown that silently never updates again.
   */
   ...
   * compose tab and opens this popup, which claims it here. The background
   * drops the text as it hands it over, so a toolbar or shortcut open - which
   * parks nothing - gets an empty string and the popup opens empty.
   *
   * The prefill is the convenient path, not the reliable one. `selectionText`
   * is plain text extracted from HTML, so whatever indentation it arrives with
   * is whatever survived that extraction. Pasting over it is still the path
   * that gives the block its indentation back.
   */
  async function claimSelectionPrefill() {
    ...
    // Assigning `value` from script fires no `input` event, so the change has
    // to be announced by hand - once, to the one thing that watches the
    // textarea. Content that arrived from outside is as wholesale as a paste,
    // and it is already here rather than being typed, so there is no burst to
    // wait out.
    handleSourceChanged({ wholesale: true, immediate: true });
  }
  ...
  // Ctrl+Enter confirms, so paste-and-insert never needs the mouse. Bound on
  // the document rather than the textarea so it also works once focus has moved
  // to the button.
  //
